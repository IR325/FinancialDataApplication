...
MEMO = "20240206_02_no_oputuna.pyをベース.承認日と支払日の年月をカテゴリ変数に."
...
class Params:
    ...
    model_default_weights = [0.521856, 0.264442, 0.213702]
...
def preprocess_data(df: pd.DataFrame) -> pd.DataFrame:
    ...
    for col in ymd_cols:
        df[col + "_year"] = pd.to_datetime(df[col]).apply(lambda x: x.year).astype(str)
        df[col + "_month"] = pd.to_datetime(df[col]).apply(lambda x: x.month).astype(str)
...
def preprocess_data_for_nn(df_train: pd.DataFrame, df_test: pd.DataFrame) -> tuple[pd.DataFrame, pd.DataFrame]:
    ...
    df_train[Params.categorical_features] = df_train[Params.categorical_features].astype("category").copy()
    df_test[Params.categorical_features] = df_test[Params.categorical_features].astype("category").copy()
