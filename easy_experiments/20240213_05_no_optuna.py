...
MEMO = "20240213_04_no_optuna.pyをベース．Sectorごとの集約特徴量を作成．"
...
def add_aggregate_feature(df, groupby_col: str, agg_cols: str):
    for agg_col in agg_cols:
        ...
        df[f"std_{groupby_col}_{agg_col}"] = (df[agg_col] - df[f"average_{groupby_col}_{agg_col}"]) / df[f"average_{groupby_col}_{agg_col}"]
    return df
...
def add_feature(df: pd.DataFrame, interest_data: pd.DataFrame) -> pd.DataFrame:
    ...
    df = add_aggregate_feature(df, groupby_col="Sector", agg_cols=agg_cols)
    # frequency encoding
    df[Params.categorical_features] = df[Params.categorical_features].fillna("Unknown").astype("category").copy()
