...
def encode_data(train_data, test_data):
    ...
    train_data["NoEmp"] = train_data["NoEmp"] + 1
    test_data["NoEmp"] = test_data["NoEmp"] + 1
    average_dict = dict(train_data.groupby(["Sector"])["NoEmp"].mean())
    std_dict = dict(train_data.groupby(["Sector"])["NoEmp"].std())

    train_data["AverageNoEmp"] = train_data["Sector"].map(average_dict)
    test_data["AverageNoEmp"] = test_data["Sector"].map(average_dict)
    train_data["StdNoEmp"] = train_data["Sector"].map(std_dict)
    test_data["StdNoEmp"] = test_data["Sector"].map(std_dict)

    train_data["DiffRateNoEmp"] = (train_data["NoEmp"] - train_data["AverageNoEmp"]) / train_data["AverageNoEmp"]
    test_data["DiffRateNoEmp"] = (test_data["NoEmp"] - test_data["AverageNoEmp"]) / test_data["AverageNoEmp"]
    train_data["StdDiffRateNoEmp"] = (train_data["NoEmp"] - train_data["AverageNoEmp"]) / train_data["StdNoEmp"]
    test_data["StdDiffRateNoEmp"] = (test_data["NoEmp"] - test_data["AverageNoEmp"]) / test_data["StdNoEmp"]
