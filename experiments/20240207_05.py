...
class Params:
    ...
    n_trials = 100
    seed = 42
    methods = ["LightGBM", "CatBoost", "NN", "RandomForest"]
    # 前処理関連
    drop_cols = []
...
def preprocess_data_for_rf(df_train, df_test):
    df_train_for_rf = df_train.drop(columns=["MIS_Status"] + Params.drop_cols_for_nn).copy()
    df_test_for_rf = df_test.drop(columns=Params.drop_cols_for_nn).copy()

    # カテゴリ変数をラベルエンコーディング
    category_cols = df_train_for_rf.select_dtypes("category").columns
    oe = OrdinalEncoder(handle_unknown="use_encoded_value", unknown_value=100000)
    oe.fit(df_train_for_rf[category_cols])
    train_categorical_data = oe.transform(df_train_for_rf[category_cols])
    df_train_categorical_for_rf = pd.DataFrame(
        data=train_categorical_data, columns=oe.get_feature_names_out() + "_rf", index=df_train_for_rf.index
    ).fillna(0)
    test_categorical_data = oe.transform(df_test_for_rf[category_cols])
    df_test_categorical_for_rf = pd.DataFrame(
        data=test_categorical_data, columns=oe.get_feature_names_out() + "_rf", index=df_test_for_rf.index
    ).fillna(0)

    # 結合
    df_train = pd.concat([df_train, df_train_categorical_for_rf], axis=1)
    df_test = pd.concat([df_test, df_test_categorical_for_rf], axis=1)
    return df_train, df_test
...
def get_params(method, trial):
    if method == "lightgbm":
        ...
        if DO_OPTUNA:
            if trial:
                params = Params().get_rf_params_range(trial)
            else:
                if Params.cv_best_params:
                    best_params = {k.split("rf_")[1]: v for k, v in Params.cv_best_params.items() if "rf" in k}
                else:
                    best_params = {k.split("rf_")[1]: v for k, v in Params.study.best_params.items() if "rf" in k}
                params = Params.rf_constant_params | best_params
        else:
            params = Params.rf_constant_params | Params.rf_default_params
    return params
...
def rf_training(X_train, y_train, X_eval, y_eval, trial):
    params = get_params("rf", trial)
...
def _extract_nn_cols(df):
    return [col for col in df.columns if "_nn" in col]


def _extract_not_nn_cols(df):
    return [col for col in df.columns if "_nn" not in col]


def _extract_rf_cols(df):
    return [col for col in df.columns if "_rf" in col]
...
def train(method, X_train, y_train, X_eval, y_eval, trial):
    if method == "LightGBM":
        ...
        use_cols = _extract_rf_cols(X_train)
        model = rf_training(X_train[use_cols], y_train, X_eval[use_cols], y_eval, trial)
    return model
...
def Preprocessing():
    ...
    # RF用前処理
    train_data, test_data = preprocess_data_for_rf(train_data, test_data)
