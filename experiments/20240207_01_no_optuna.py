...
MEMO = "20240206_02_no_optuna.pyをベース．データ分割方法をStratifiedKfoldにしてみる"
...
def cv_training(methods, X, y):
    ...
    best_params_arr = []
    scores = []
    kf = StratifiedKFold(n_splits=Params.n_splits, random_state=42, shuffle=True)
    for i, (train_eval_tune_index, valid_index) in enumerate(kf.split(X, y)):
        # データの分割
