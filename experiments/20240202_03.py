...
MEMO = "optunaの導入.フランチャイズコードの処理を追加"
...
def Preprocessing():
    ...
    for col in Params.encoding_target_cols:
        ...
        train_data[f"{col}_freq_encoding"] = train_data[col].map(train_count_dict).astype(int)
        test_data[f"{col}_freq_encoding"] = test_data[col].map(test_count_dict).astype(int)
