...
# 個別設定
EXPERIMENT_NAME = "20240121_baseline"
...
def main():
    ...
    # 説明変数と目的変数に分ける
    X = train_data.drop(columns="MIS_Status").select_dtypes(include=int).copy()
    y = train_data["MIS_Status"].copy()
    X_test = test_data.select_dtypes(include=int).copy()
    ...
    if os.path.exists(summary_path):
        ...
        df = pd.DataFrame({"experiment_name": [], "score": [], "MEMO": []})
    result_df = pd.DataFrame({"experiment_name": [EXPERIMENT_NAME], "score": [np.mean(scores)], "MEMO": [MEMO]})
