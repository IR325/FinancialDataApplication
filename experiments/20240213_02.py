...
MEMO = "20240212_03.pyをベース．cvされたモデルを全てアンサンブルにつかう"
...
def get_params(method, trial):  # TODO: 修正
    if method == "lightgbm":
        if DO_OPTUNA:
            if trial:  # optuna用
...
def cv_training(methods, X, y):
    models = np.zeros((Params.n_splits, len(methods))).tolist()
    ...
    for i, (train_eval_tune_index, valid_index) in enumerate(kf.split(X)):
        ...
        for j, method in enumerate(methods):
            ...
            best_weights_arr[i][j] = best_weight
        valid_pred_proba = np.average(valid_pred_probas, axis=0, weights=best_weights_arr[i])
    ...
    return models, scores, best_weights_arr, best_negative_ratios
...
def Predicting(X_test, methods, models, best_weights, best_negative_ratios):
    test_pred_probas = np.zeros((Params.n_splits, len(methods), X_test.shape[0]))
    for i in range(Params.n_splits):
        for j, method in enumerate(methods):
            test_pred_probas[i][j] = predict(method, models[i][j], X_test)
    # 予測
    best_weights_tiled = best_weights[:, :, np.newaxis]
    for k in range(X_test.shape[0] - 1):
        best_weights_tiled = np.concatenate([best_weights_tiled, best_weights[:, :, np.newaxis]], axis=2)
    y_pred_proba = np.average(test_pred_probas, axis=1, weights=best_weights_tiled)
    y_pred_proba = np.average(y_pred_proba, axis=0)
    # 後処理
    y_pred = postprocess_prediction(y_pred_proba, np.mean(best_negative_ratios))
