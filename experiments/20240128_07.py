...
MEMO = "20240128_06をベースにして，欠損値処理＋特徴量をさらに増やす."


def _encode_with_column(train_data, test_data, target_col, group_col):
    average_dict = dict(train_data.groupby([group_col])[target_col].mean())
    std_dict = dict(train_data.groupby([group_col])[target_col].std())

    train_data[f"Average{target_col}"] = train_data[group_col].map(average_dict)
    test_data[f"Average{target_col}"] = test_data[group_col].map(average_dict)
    train_data[f"Std{target_col}"] = train_data[group_col].map(std_dict)
    test_data[f"Std{target_col}"] = test_data[group_col].map(std_dict)

    train_data[f"DiffRate{target_col}"] = (train_data[target_col] - train_data[f"Average{target_col}"]) / train_data[f"Average{target_col}"]
    test_data[f"DiffRate{target_col}"] = (test_data[target_col] - test_data[f"Average{target_col}"]) / test_data[f"Average{target_col}"]
    train_data[f"StdDiffRate{target_col}"] = (train_data[target_col] - train_data[f"Average{target_col}"]) / train_data[f"Std{target_col}"]
    test_data[f"StdDiffRate{target_col}"] = (test_data[target_col] - test_data[f"Average{target_col}"]) / test_data[f"Std{target_col}"]
...
def encode_data(train_data, test_data):
    ...
    # Sectorによる従業員数関連のencoding
    train_data["NoEmp"] += 1
    test_data["NoEmp"] += 1
    train_data, test_data = _encode_with_column(train_data, test_data, target_col="NoEmp", group_col="Sector")
    # Sectorによる融資期間のencoding
    train_data, test_data = _encode_with_column(train_data, test_data, target_col="Term", group_col="Sector")
    # Sectorによる融資額のencoding
    train_data, test_data = _encode_with_column(train_data, test_data, target_col="DisbursementGross", group_col="Sector")
    # Sectorによる融資額 / 融資期間のencoding
    train_data["DisbursementGross_per_month"] = train_data["DisbursementGross"] / train_data["Term"]
    test_data["DisbursementGross_per_month"] = test_data["DisbursementGross"] / test_data["Term"]
    train_data, test_data = _encode_with_column(train_data, test_data, target_col="DisbursementGross", group_col="Sector")

    # category型への変換（本当はpreprocess_dataにおきたい）
    obj_cols = train_data.select_dtypes(include=object).columns
    train_data[obj_cols] = train_data[obj_cols].astype("category").copy()
    test_data[obj_cols] = test_data[obj_cols].astype("category").copy()
    return train_data, test_data
...
def preprocess_data(df: pd.DataFrame) -> pd.DataFrame:
    ...
    for col in ["RevLineCr", "LowDoc", "BankState"]:
        df[col] = df[col].fillna("[UNK]").copy()
    # 数値系の特徴量をいじる
    df["CreateJobRatio"] = df["CreateJob"] / df["NoEmp"]
    df["EmpDiff"] = df["NoEmp"] - df["RetainedJob"]
    # 金額をobj->int型へ変換
