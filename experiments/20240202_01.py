...
MEMO = "テストデータについては，テストデータのみでencodingする."
...
class Params:
    ...
    drop_cols = []
    categorical_features = ["FranchiseCode", "RevLineCr", "LowDoc", "UrbanRural", "State", "BankState", "Sector", "City"]
    encoding_target_cols = ["FranchiseCode", "RevLineCr", "LowDoc", "UrbanRural", "State", "BankState", "Sector", "City"]
    weights = [comb for comb in list(itertools.product(np.arange(0, 1.01, 0.1), repeat=len(methods))) if sum(comb) == 1]
...
def Preprocessing():
    ...
    for col in Params.encoding_target_cols:
        train_count_dict = dict(train_data[col].value_counts())
        test_count_dict = dict(test_data[col].value_counts())
        train_data[f"{col}_freq_encoding"] = train_data[col].map(train_count_dict)
        test_data[f"{col}_freq_encoding"] = test_data[col].map(test_count_dict)
    # 前処理
